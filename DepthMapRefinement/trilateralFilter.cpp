...
void trilateralWeightMapBase_32f( const Mat& src, const Mat& guide, Mat& dst, int d,
	...
	double sigma_color, double sigma_guide_color, double sigma_space,int borderType)
{
	...
	{
		trilateralWeightMapBase_32f(src,guide,dst,d,sigma_color,sigma_guide_color,sigma_space,borderType);
	}
	else
	{
		Mat ss,gg;
...
void trilateralFilter( const Mat& src,const Mat& joint, Mat& dst, int d, double sigma_color, double sigma_joint_color, double sigma_space,int borderType)
{
	if(src.type()==CV_MAKE_TYPE(CV_8U,src.channels()))
	{
		trilateralFilterBase_8u(src,joint,dst,d,sigma_color,sigma_joint_color, sigma_space,borderType);
	}
	else if(src.type()==CV_MAKE_TYPE(CV_32F,src.channels()))
	{
		trilateralFilterBase_32f(src,joint,dst,d,sigma_color,sigma_joint_color, sigma_space,borderType);
	}
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void trilateralFilter_32f( const Mat& src, const Mat& guide, Mat& dst, Size kernelSize , double sigma_color, double sigma_guide_color, double sigma_space, int borderType )
{
...
void trilateralFilter_8u( const Mat& src, const Mat& guide, Mat& dst, Size kernelSize , double sigma_color, double sigma_guide_color,double sigma_space, int borderType )
{
	if(kernelSize.width==0 || kernelSize.height==0){ src.copyTo(dst);return;}
	int cn = src.channels();
	int cng = guide.channels();
...
void trilateralFilterSP_32f( const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color,double sigma_space, int borderType )
{
	trilateralFilter_32f(src, guide, dst, Size(kernelSize.width,1), sigma_color, sigma_guide_color, sigma_space, borderType );
	trilateralFilter_32f(dst, guide, dst, Size(1,kernelSize.height), sigma_color, sigma_guide_color, sigma_space, borderType );
}
void trilateralFilterSP_8u( const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color,double sigma_space, int borderType )
{
	trilateralFilter_8u(src, guide, dst, Size(kernelSize.width,1), sigma_color, sigma_guide_color, sigma_space, borderType );
	trilateralFilter_8u(dst, guide, dst, Size(1,kernelSize.height), sigma_color, sigma_guide_color, sigma_space, borderType );
}

void trilateralFilter(const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color,double sigma_space,int method, int borderType)
{
	...
	if(method==BILATERAL_NORMAL)
	{
		if(src.type()==CV_MAKE_TYPE(CV_8U,src.channels()))
		{
			trilateralFilter_8u(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
		}
		else if(src.type()==CV_MAKE_TYPE(CV_32F,src.channels()))
		{
			trilateralFilter_32f(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
		}
	}
	else if(method==BILATERAL_SEPARABLE)
	{
		if(src.type()==CV_MAKE_TYPE(CV_8U,src.channels()))
		{
			trilateralFilterSP_8u(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
		}
		else if(src.type()==CV_MAKE_TYPE(CV_32F,src.channels()))
		{
			trilateralFilterSP_32f(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
		}
	}
}
...
void trilateralWeightMap_32f( const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color, double sigma_space, int borderType )
{
	int cn = src.channels();
	int cng = guide.channels();
	int i, j, maxk;
...
void trilateralWeightMap_8u( const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color, double sigma_space, int borderType )
{
	int cn = src.channels();
	int cng = guide.channels();
	int i, j, maxk;
...
void trilateralWeightMapSP_32f( const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color,double sigma_space, int borderType )
{
	trilateralWeightMap_32f(src, guide, dst, Size(kernelSize.width,1), sigma_color, sigma_guide_color, sigma_space, borderType );
	trilateralWeightMap_32f(dst, guide, dst, Size(1,kernelSize.height), sigma_color, sigma_guide_color, sigma_space, borderType );
}
void trilateralWeightMapSP_8u( const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color,double sigma_space, int borderType )
{
	trilateralWeightMap_8u(src, guide, dst, Size(kernelSize.width,1), sigma_color, sigma_guide_color, sigma_space, borderType );
	trilateralWeightMap_8u(dst, guide, dst, Size(1,kernelSize.height), sigma_color, sigma_guide_color, sigma_space, borderType );
}

void trilateralWeightMap(const Mat& src, const Mat& guide, Mat& dst, Size kernelSize, double sigma_color, double sigma_guide_color,double sigma_space,int method, int borderType)
{
	...
	if(method==BILATERAL_NORMAL)
	{
		if(src.type()==CV_MAKE_TYPE(CV_8U,src.channels()))
		{
			trilateralWeightMap_8u(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
		}
		else if(src.type()==CV_MAKE_TYPE(CV_32F,src.channels()))
		{
			trilateralWeightMap_32f(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
		}
	}
	else if(method==BILATERAL_SEPARABLE)
	{
		if(src.type()==CV_MAKE_TYPE(CV_8U,src.channels()))
		{
			trilateralWeightMapSP_8u(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
		}
		else if(src.type()==CV_MAKE_TYPE(CV_32F,src.channels()))
		{
			trilateralWeightMapSP_32f(src,guide,dst,kernelSize,sigma_color,sigma_guide_color, sigma_space,borderType);
		}
	}
}